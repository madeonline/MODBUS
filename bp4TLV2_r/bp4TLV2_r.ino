 ...
 * set = 1;                          // Menu 1/5 , выбор валкодером режимов normal/shundown/drop
 * set = 2;                          // Menu 2/5 , установка валкодером максимального тока
 * set = 3;                          // Menu 3/5 , Сброс настроек по току ?
 * set = 4;                          // Menu 4/5 , Сохранить настройки
 * set = 5;                          // Menu 5/5 , Новый пункт меню - выбор ШИМ
 * set = 6;                          // режим защиты по току,
 * set = 7;                          // ЗАЩИТА и выключение
 *
...
void loop()                                                //основной цикл работы МК
{
  ...
  if (digitalRead(kn_menu) == LOW && knopka_b == 0)                   // Переключение режимов меню кнопкой "меню"
  {
    knopka_b = 1;
    set++;                                                            //
    if (set > 5 | off)                                                // Задействован один из режимов защиты, а этой кнопкой мы его вырубаем. (или мы просто дошли до конца меню)
    {
      off = false;
      digitalWrite(power, HIGH);                                      // врубаем реле если оно было выключено
      delay(100);
