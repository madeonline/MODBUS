...
void uup()                                                     //энкодер +
{
  ...
  {
    mode++;
    if (mode > 2) mode = 2;                                    //дошли до конца
  }
...
{
 //
 if (pwm1_2)                                                        // Регулировка при включенном ШИМ1
	{
 ...
 {
		//if (Uout > counter2)                                        //Сравниваем напряжение на выходе с установленным, и принимаем меры..
		//{
		//  float raz = Uout - counter2;                              //на сколько напряжение на выходе больше установленного...
		//  if (raz > 0.05)
		//  {
		//	level2 = level2 - raz * 20;                             //разница большая управляем грубо и быстро!
		//  } else {
		//	if (raz > 0.015)  level2 = level2 -  raz * 3 ;          //разница небольшая управляем точно
		//  }
		//}
		//if (Uout < counter2)
		//{
		//  float raz = counter2 - Uout;                              //на сколько напряжение меньше чем мы хотим
		//  if (raz > 0.05)
		//  {
		//	level2 = level2 + raz * 20; //грубо
		//  } else {
		//	if (raz > 0.015)  level2 = level2 + raz * 3 ;           // точно
		//  }
		//}

	  analogWrite(pwm2, ceil(counter2));               //подаем нужный сигнал на ШИМ2 выход (кроме 255, так как там какая-то лажа)
 }

  if (off) level1 = 8190;
  if (level1 < 0) level1 = 0;                                             //не опускаем ШИМ1 ниже нуля
  if (level1 > 8190) level1 = 8190;                                       //не поднимаем ШИМ1 выше 13 бит
  if (level2 < 0) level2 = 0;                                             //не опускаем ШИМ2 ниже нуля
  if (level2 > 8190) level2 = 8190;                                       //не поднимаем ШИМ2 выше 13 бит                                                                       //Все проверили, прощитали и собственно отдаем команду для силового транзистора.

  if (ceil(level1) != 255) analogWrite(pwm1, ceil(level1));               //подаем нужный сигнал на ШИМ1 выход (кроме 255, так как там какая-то лажа)
  
  analogWrite(pwm2, ceil(level2));               //подаем нужный сигнал на ШИМ2 выход (кроме 255, так как там какая-то лажа)
}                               // УПРАВЛЕНИЕ                            
