...
volatile unsigned int  level1  = 32768;          // Средний "уровень" ШИМ сигнала.  (volatile) указываем что переменная работает с прерываниями
volatile unsigned int  level2  = 32768;          // Средний "уровень" ШИМ сигнала.  (volatile) указываем что переменная работает с прерываниями
float com           = 300;
long com2           = 0;
...
//  lcd.backlight();  
 ...
 //level2 = EEPROM_float_read(20);
  ...
  MsTimer2::set(20, flash_time);                // Настраиваем таймер 20 ms период таймера прерывания для опроса валкодера (время необходимо подстроить по скорости валкодера) !! 20 ms это так быстро как
...
void loop()                                   // основной цикл работы МК
{
  ...
  float Ucorr = 0.00;    
  // коррекция напряжения, при желании можно подстроить
  Uout_temp += analogRead(A1) * ((5.0 + Ucorr) / 1023.0) * 5.0;         // узнаем напряжение на выходе 
  Iout_temp += analogRead(A0) / 100.00;                                 // узнаем ток в нагрузке
 ...
 // if (((Iout>(level1)*2.0) | Iout>10.1  | off) & set<4 & millis()>100 ) // условия защиты
  if (( Iout>10.1  | off) & set<4 & millis()>100 ) // условия защиты
   {  
