...
volatile unsigned int pulses_temp = 0;         // Счетчик временного хранения количества импульсов мотора B
...
void setup()
{
  ...
  if (analogRead(stop_motorB) > 20)
  {
    Serial.println("Motor B parking waiting");     // Вывод в серийный порт сообщения о парковке мотора В
    digitalWrite(motorB_in3, LOW);               // Запуск двигателя В влево
    digitalWrite(motorB_in4, HIGH);              // Запуск двигателя В влево
    analogWrite(motorB_en, 200);                 // устанавливаем  скорость из доступного диапазона 0~255. При плавном пуске необходимо закомментировать строку
    while (analogRead(stop_motorB) > 20) {}      // Ожидаем сигнал с датчика парковки мотора В
   }
  else
  {
    ...
    Serial.println("Motor B parking");           // Вывод в серийный порт сообщения о парковке мотора В
  }
...
void loop()
{
  ...
  pulses_temp = 0;                       // Сбрасываем  временный счетчик импульсов
  ...
  delay (1000);                            // Проверяем 1 раз в секунду. Разрешено не более 7 секунд  иначе перейдет в сброс.
  wdt_reset();                            // говорим собаке что "В Багдаде все спокойно", начинается очередной отсчет 8-и секунд.
}
