...
byte  level2        = 200;                       //"уровень" ШИМ сигнала
float com           = 100;
long com2           = 0;
...
{          
	...
	pinMode(pwm, OUTPUT);  // На выход 9
	pinMode(pwm, OUTPUT);  // На выход 6
	pinMode(down, INPUT);  
	pinMode(up, INPUT);  
...
analogWrite(pwm1, level2); //подаем нужный сигнал на ШИМ выход (кроме 255, так как там какая-то лажа)
